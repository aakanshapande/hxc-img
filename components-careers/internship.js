...
const InternshipAndListings = ({ jobs = [] }) => {
  ...
  // Filter jobs by search and selected category
        ...
        >
          <div className="w-full flex justify-center mb-10">
            <span className="inline-block" style={{ background: badgeColor, color: 'white', fontSize: '1.875rem', fontWeight: 700, padding: '1rem 2.5rem', borderRadius: '1rem' }}>
              {t('internship_programs_title', 'Internship Programs Hakxcore.io')}
            </span>
          </div>
          <div className="flex flex-col md:flex-row items-center w-full mb-8">
            <div className="flex-1 flex flex-col justify-center items-start md:pl-8">
              <h2 className="text-5xl md:text-5xl font-bold mb-4 leading-tight text-black text-left">
                {t('get_your_experience', 'Get Your Experience')} <br /> {t('you_need', 'You Need')}
              </h2>
            </div>
            <div className="flex-1 flex justify-center md:justify-end w-full md:pr-8 mt-8 md:mt-0">
              <Image
                ...
                alt={t('internship_search_illustration', 'Internship search illustration')}
                width={220}
                height={160}
          ...
          <div className="mt-2 mb-2 w-full text-center">
            <p className="text-3xl font-bold text-black">
              {t('choose_category', 'Choose Your Category In Your Passionate Cybersecurity Subfield')}
            </p>
          </div>
      ...
      <div className="w-full flex justify-center bg-[#f7fafc] py-16 mt-16 flex-col items-center">
        <div className="w-full max-w-7xl">
          <div className="text-5xl font-bold text-gray-900 text-center mb-8">
            {filteredJobs.length > 0 ? t('open_positions', { count: filteredJobs.length, defaultValue: 'We have {{count}} open position{{count, plural, one {} other {s}}} now!' }) : t('no_open_positions', 'No open positions at the moment.')}
          </div>
        </div>
          ...
          <aside className="w-72 flex-shrink-0">
            <div className="bg-white rounded-md shadow-sm p-6">
              <div className="flex items-center mb-8">
                ...
                <h2 className="text-2xl font-bold">{t('all_positions', 'All positions')} <span className="font-normal">({filteredJobs.length})</span></h2>
              </div>
